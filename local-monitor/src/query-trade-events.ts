...
import { CONTRACT_ADDRESSES, DEPLOYMENT_BLOCK } from "./config";

async function getProvider(): Promise<ethers.JsonRpcProvider> {
  const rpc =
    process.env.MAINNET_RPC_HTTP_URL ||
    process.env.RPC_HTTP_URL ||
    "https://eth.llamarpc.com";
  return new ethers.JsonRpcProvider(rpc);
}

const CORE_ABI = JSON.parse(
  fs.readFileSync(path.join(__dirname, "abi/Core.json"), "utf8")
);
...
async function main() {
  ...
  const events = await core.queryFilter(
    streamFilter,
    DEPLOYMENT_BLOCK,
    24490798 + 1000 // past the failed block
  );

  const completedFilter = core.filters.TradeCompleted(tradeId);
  const completedEvents = await core.queryFilter(
    completedFilter,
    DEPLOYMENT_BLOCK,
    24490798 + 1000
  );
  ...
  if (createdEvents.length > 0) {
    const ce = createdEvents[0] as ethers.EventLog;
    const cargs = ce.args as unknown as {
      amountIn?: bigint;
  ...
  events.sort(
    (a, b) =>
      ...
      (a as ethers.EventLog).index - (b as ethers.EventLog).index
  );
  ...
  events.forEach((evt, i) => {
    ...
    const args = log.args as unknown as { tradeId: bigint; amountIn: bigint; realisedAmountOut: bigint; lastSweetSpot: bigint };
    ...
    const sweetSpotUsed =
      lastSweetSpot === 0 ? 1 : lastSweetSpot + 1;
  ...
  const isStrict4321 =
    ...
    sweetSpots[3] === 1;
  console.log(
    `   Strict 4→3→2→1 pattern: ${isStrict4321 ? "YES" : "NO"}`
  );
  ...
  console.log(
    `   amountRemaining after: ${(amountIn - totalStreamed).toString()}`
  );
  ...
  if (amountIn - totalStreamed === 0n) {
    ...
    console.log(
      "   → Trade should have been completed/removed. If still stuck, completion logic may have failed."
    );
  } else {
    console.log(
  ...
  const totalOut = events.reduce((acc, evt) => {
    const log = evt as ethers.EventLog;
    const args = log.args as unknown as { realisedAmountOut?: bigint };
  ...
  events.forEach((evt, i) => {
    const log = evt as ethers.EventLog;
    const args = log.args as unknown as { amountIn: bigint; realisedAmountOut: bigint };
  ...
  if (events.length > 0) {
    const firstTxHash = (events[0] as ethers.EventLog).transactionHash;
    const receipt = await provider.getTransactionReceipt(firstTxHash);
    const iface = new ethers.Interface(CORE_ABI.filter((x: { type: string }) => x.type === 'event'));
    const coreLogs = receipt?.logs.filter((l) => l.address.toLowerCase() === CONTRACT_ADDRESSES.core.toLowerCase()) ?? [];
  ...
  if (completedEvents.length > 0) {
    completedEvents.forEach((evt, i) => {
      const log = evt as ethers.EventLog;
      console.log(
        `   #${i + 1}: block ${log.blockNumber}, tx ${log.transactionHash}, finalRealised=${log.args?.finalRealisedAmountOut?.toString()} wei`
      );
    });
  }
