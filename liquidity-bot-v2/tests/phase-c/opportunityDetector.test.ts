...
function mockQuoteService(opts: {
  ...
  sellBaseOut: bigint;
  /** Full-size alt in for linear probe scaling (default matches test buy out). */
  fullAltIn?: bigint;
}): DexQuoteService {
  const fullAltIn = opts.fullAltIn ?? 1_150_000n;
  ...
  const quoteDex = vi.fn(
    async (
      ...
      if (
        ...
        const inAmt = amountIn ?? fullAltIn;
        // Prefer deep venue on the sell so usage check uses 20M reserve.
        const fullOut =
          dex === 'uniswap-v3-10000' ? opts.sellBaseOut : opts.sellBaseOut - 30_000n;
        const amountOut =
          fullAltIn > 0n ? (fullOut * inAmt) / fullAltIn : fullOut;
        return {
          dex,
          amountOut,
          liquidityScore: 1n,
