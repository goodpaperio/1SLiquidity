...
import { startCexWatchForBot } from '../signal/cexWs.js';
...
export class BotRunner {
  ...
  private cexWatchStop: (() => void) | null = null;
  ...
  stop(): void {
    ...
    if (this.cexWatchStop) {
      this.cexWatchStop();
      this.cexWatchStop = null;
    }
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      if (
        this.config.scan.universeMode === 'hot_pairs' &&
        this.config.scan.watchMode !== 'off'
      ) {
        try {
          await resolveHotPairsForBot(this.config);
          const watch = await startCexWatchForBot(this.config.id);
          this.cexWatchStop = watch?.stop ?? null;
          console.log(
            `[${id}] cexWatch ${this.cexWatchStop ? 'started' : 'unavailable (REST each cycle)'}`
          );
        } catch (err) {
          console.warn(
            `[${id}] cexWatch failed:`,
